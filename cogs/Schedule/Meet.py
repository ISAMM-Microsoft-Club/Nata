...
class Meet:
  ...
  async def meet(self):
    await self.ctx.send(f"{self.dept} Department meeting scheduled on {self.date} at {self.time}. ( by {self.bot.config.chefs[str(self.chef)]['position']} )")
    print(self.deltaTime)
    await asyncio.sleep(self.deltaTime)
    channel = self.bot.get_channel(1010310116257574916)
    present__report = []
    present = []
